...
import {
  StyledBusinessSectionWrapper, StyledOtherDocumentationSectionWrapper,
...
import {StyledButtonGetStarted, StyledButtonLearnMore} from '@/components/UI/Button/Button.styled';
...
function SubSection({title, grid}) {
  return (
    <StyledSubSectionWrapper className="sub-section">
      <StyledTypographyUrbanistH3 className="title">{title}</StyledTypographyUrbanistH3>
      <div className="grid">
        {grid.map(({id, icon, title: titleCart, answers, listQuestions}) =>
          <li className="cart" key={id}>
            <div className="cart-header">
              {icon}
              <StyledTypographyUrbanistH5 className="cart-title">{titleCart}</StyledTypographyUrbanistH5>
            </div>
            <div className="cart-body">
              {listQuestions.map(({
                ...
                <StyledTypographyUrbanistBody key={idQuestion} className="cart-question">
                  {text}
                </StyledTypographyUrbanistBody>
              )}
            </div>
            <div className="cart-footer">
              <StyledButtonLearnMore className="cart-btn-view">View all</StyledButtonLearnMore>

              <StyledTypographyUrbanistBody
                className="cart-answers-text">{answers} answers</StyledTypographyUrbanistBody>
            </div>
          </li>
        )}
      </div>
...
function OtherDocumentationSection({title, titleCart, descriptionCart, btnText}) {
  return (
    <StyledOtherDocumentationSectionWrapper>
      ...
      <div className="cart">
        ...
        <div className="right-side">
          <Image src={image.src} alt={titleCart} width={128.59} height={146.13}/>
        </div>
      </div>
    </StyledOtherDocumentationSectionWrapper>
  )
